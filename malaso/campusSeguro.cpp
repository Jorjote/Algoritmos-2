...
campusSeguro::campusSeguro(campus c, Dicc<Agente,Posicion> d)
{
    ...
    diccHash<Agente,campusSeguro::Datos>::Iterador itHashVacio;
  ...
  while( it.HaySiguiente() )
  {
        ...
        data.tag = "agente";
        data.placa = plak;
        data.datosAgente = dicc.Definir(plak, dataH); 
        cout << "ACAAAAA : " << itEqu.claseActual() << endl;// definir retorna un iterador de Dicc.
    ...
    it2 = d.CrearIt();
    cout << equivalenciaSanciones << " LEEER ACAAA " << endl;
...
void campusSeguro::moverEstudiante(Nombre e,Direccion dir)
{
    ...
    if( seRetira(posActual,dir) )
    {
        ...
        diccHash<Agente,Datos>::Iterador itNulo;// creo un iterador vacio.
...
{
    //cout << equivalenciaSanciones << " LEEEE RRRR" << endl;
